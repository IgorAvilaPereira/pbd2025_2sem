...
public class SessaoDAO {
    ...
    public Sessao obter(int sessao_id) throws SQLException {
        Connection conexao = new ConexaoPostgreSQL().getConexao();
        PreparedStatement instrucao = conexao.prepareStatement("select sessao.id, sessao.data, sessao.hora_inicio, sessao.hora_fim, filme.id as filme_id, filme.titulo as filme_titulo, sala.id as sala_id from sessao join filme on filme.id = sessao.filme_id join sala on sala.id = sessao.sala_id where sessao.id = ?; ");
        instrucao.setInt(1, sessao_id);
        ResultSet rs = instrucao.executeQuery();
        Sessao sessao = null;
        if (rs.next()) {
            sessao = new Sessao();
